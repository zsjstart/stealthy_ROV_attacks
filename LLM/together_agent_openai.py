...
from openai_agent import analyze_with_ChatOpenAI_model
import pickle
...
def together_agent():
    ...
    label = "openai"
    model_name = label+"/gpt-oss-20b"
    
    # CSV file path
    csv_file = "./new_results/origin_conflicts/"+label+"_reasoning_origin_conflicting_routes.txt"
    ...
    #consider possible AS path manipulation in case of analyzing AS path. Also, you may need to take account of possible errors in CAIDA data
    for i in range(0, len(origin_conflicting_routes)):
        try:
            ...
            #f = open('./new_results/origin_conflicts/gemini_reasoning_origin_conflicting_routes_('+str(num)+').txt', 'w')
            ...
            query = """
                ...
                - If a ROA maxLength mismatch exists, evaluate whether it could be a path-manipulation attack with a valid origin.
            ...
            print(f"[ERROR] Failed to {label} reasoning: {e}; {response}")
